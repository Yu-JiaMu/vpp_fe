    ...
    <div class="params-form">
      <div class="params-section">
        ...
        <div v-for="item in inputParams" :key="item.key">
          <el-form-item
              v-if="item.needId.includes(apiInfo.id)"
              :prop="item.key"
              class="param-item"
          >
...
const syncModel = () => {
  props.inputParams.forEach(item => { formModel[item.key] = item.value })
  props.pageParams.forEach(item => { formModel[item.key] = item.value })
}
syncModel()

watch(() => props.inputParams, () => {
  props.inputParams.forEach(item => { formModel[item.key] = item.value })
}, { deep: true })
...
const formRules = computed(() => {
  ...
  props.inputParams.forEach(item => {
    if (item.needId.includes(props.apiInfo.id) && item.isRequired.includes(props.apiInfo.id)) {
      rules[item.key] = [{ required: true, message: `${item.cnLabel || item.label}不能为空`, trigger: ['blur', 'change'] }]
    }
  })
  props.pageParams.forEach(item => {
    if (item.required) {
...
.params-form {
  ...
  &::-webkit-scrollbar-thumb { background: #dcdfe6; border-radius: 3px; &:hover { background: #c0c4cc; } }
...
:deep(.date-range-picker) { width: 100% !important; }
